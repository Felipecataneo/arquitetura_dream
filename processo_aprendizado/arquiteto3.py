# ==============================================================================
#           SISTEMA DREAM V6.5: FRAMEWORK ADAPTATIVO GENERALISTA
#               COM DOMÍNIO PROBABILÍSTICO E VERIFICAÇÃO DE PREMISSAS
# ==============================================================================
...
class RootCauseAnalyzer:
    ...
    def _generate_5whys_prompt(self, problem: str) -> str:
        return f"""
            ...
            Se a pergunta for mais complexa (ex: "How many pairs of twins do you need..."), a causa raiz pode ser "entender a lógica de um problema de probabilidade com pegadinha (riddle)".
...
class DomainComplexityAnalyzer:
    def __init__(self):
        self.domain_keywords = {
            "probabilistic": [
                "chance", "probability", "probabilidade", "at least", "pelo menos",
                "expected value", "dado", "moeda", "sorteio", "birthday paradox", "odds"
            ],
            "mathematical": [
                "equação", "resolver", "calcular", "número", "inteiro", "divisor", "primo", "fator",
                "menor", "maior", "múltiplo"
            ],
...
class DreamSystem:
    ...
    def _reevaluate_complexity(self, current_complexity: ComplexityLevel, whys_analysis: Dict, context: Dict) -> ComplexityLevel:
        ...
        complex_keywords = [
            ...
            "relação entre", "propriedade de", "princípio de", "fórmula para", "pegadinha", "riddle"
        ]
        
        if context.get('is_number_theory') or context.get('domain') == 'probabilistic' or any(keyword in root_cause for keyword in complex_keywords):
    ...
    def _mathematical_decomposition(self, problem: str, context: Dict) -> Dict:
        ...
        prompt = f"""
            ...
            1.  **Análise de Premissas:** Identifique todas as premissas e restrições do problema. Preste atenção especial a palavras que mudam completamente o cenário (ex: "gêmeos", "exatamente", "pelo menos").
            2.  **Plano de Resolução:** Com base nas premissas, crie um plano lógico para resolver o problema. Se houver uma pegadinha, seu plano deve se concentrar em resolvê-la diretamente.
            3.  **Execução do Plano:** Execute o plano passo a passo, explicando cada etapa.

            Responda em um formato JSON com uma chave "solution_analysis" que contém um objeto com as chaves "premises", "plan", e "execution".

            Exemplo para o problema dos gêmeos:
            {{
              "solution_analysis": {{
                "premises": [
                  "O problema pede por 'pares de gêmeos'.",
                  "Gêmeos compartilham o mesmo aniversário por definição.",
                  "A condição é ter 'pelo menos 50% de chance' de que 'duas pessoas' tenham o mesmo aniversário.",
                  "Esta é uma pegadinha. A presença de um único par de gêmeos já satisfaz a condição com 100% de certeza."
                ],
                "plan": [
                  "Identificar que a restrição 'gêmeos' anula a necessidade de cálculos de probabilidade complexos.",
                  "Concluir que um par de gêmeos (2 pessoas) já garante que duas pessoas têm o mesmo aniversário.",
                  "Declarar a resposta diretamente com base nessa lógica."
                ],
                "execution": [
                  "O problema é uma variação do paradoxo do aniversário, mas com uma pegadinha.",
                  "A palavra-chave é 'gêmeos'. Gêmeos idênticos ou não-idênticos nascem no mesmo dia.",
                  "Portanto, se houver um único par de gêmeos na sala, já existem duas pessoas com o mesmo aniversário.",
                  "A probabilidade de duas pessoas terem o mesmo aniversário em uma sala com um par de gêmeos é de 100%.",
                  "Como 100% é maior que 50%, a condição é satisfeita com o menor número possível de pares: 1.",
                  "Conclusão: Você precisa de apenas 1 par de gêmeos."
                ]
              }}
            }}
        ...
        try:
            ...
            if analysis and all(k in analysis for k in ["premises", "plan", "execution"]):
                ...
                self.meta_state.reasoning_trace.append("MATH_DECOMP: Falha no formato estruturado, recorrendo à abordagem neural.")
...
if __name__ == "__main__":
    print("="*50)
    print("🤖 Agente com Arquitetura Cognitiva Híbrida V6.5 🤖")
    print("="*50)
    ...
    print("  - Lógica/Pegadinha: How many pairs of twins do you need in a room for there to be at least a 50% chance that two people have the same birthday?")
